...
import 'package:deniz_gold/presentation/widget/Update_detail.dart';
        ...
        body: BlocProvider<SplashCubit>(
          ...
          child: BlocConsumer<SplashCubit, SplashState>(
            listener: (context, state) {
              if (state is SplashAuthenticationChecked && state.authenticated) {
              ...
              } else if (state is SplashAuthenticationChecked &&
                  !state.authenticated) {
                ...
                final dialog = ConfirmDialog(
                  ...
                  onConfirmClicked: () async {
                    context.pop();
                    await launchUrlString(state.appVersion.link,
                        mode: LaunchMode.externalApplication);
                  },
                  onCancelClicked: state.forceUpdate
                      ? null
                      : () => context.goNamed(HomeScreen.route.name!),
                );
              ...
              children: [
                ...
                Padding(
                  padding: const EdgeInsets.symmetric(
                      vertical: Dimens.standard125,
                      horizontal: Dimens.standard16),
                  child: Container(
                    decoration: const BoxDecoration(
                        color: AppColors.white,
                        borderRadius: BorderRadius.all(
                            Radius.circular(Dimens.standard160))),
                    child: const Center(
                      child: AppLogo(),
                    ),
                  ),
